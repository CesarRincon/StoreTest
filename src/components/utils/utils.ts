const key = "secretKey"
export const dataPayMethod = ["Credit Card", "Cash on delivery"]
export const dataTypeDocument = ["C.C", "C.E", "NIT"]
...
export const departments = Object.keys(dataCountry || {})

export const validatorsCustomerInfo = {
    ...
    idDocumentType: (value: string): boolean => {
        return dataTypeDocument.includes(value);
    },
};

export const cardValidators = {
    cardholderName: (value: string): boolean => {
        return /^[a-zA-Z\s]{4,}$/.test(value);
    },
    cardNumber: (value: string): boolean => {
        return /^\d{16}$/.test(value);
    },
    expiration: (value: string): boolean => {
        if (!/^(0[1-9]|1[0-2])\/\d{2}$/.test(value)) return false;
        const [month, year] = value.split("/").map(Number);
        const currentDate = new Date();
        const currentMonth = currentDate.getMonth() + 1;
        const currentYear = parseInt(String(currentDate.getFullYear()).slice(-2));
...
export const isValidForm = (data: any) => {
    return Object.values(data).every((value: any) => value?.trim() !== '');
};

export const isValidCreditCardNumber = (cardNumber: number | null) => {

    const number: string = cardNumber?.toString().replace(/\D/g, '');
...
export const getCardType = (cardNumber: number | string) => {
...
export const processSimulationCreditCardPayment = (creditCardInfo: string) => {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const creditCardDecript: any = decryptData(creditCardInfo)

            const { cardNumber, expiration, cvv, cardType } = creditCardDecript;
            const isValidExpiryDate = validateExpiryDate(expiration);
            const isValidCVV = validateCVV(cvv, cardType);

            const isValidCard = isValidCreditCardNumber(cardNumber);

            if (isValidCard && isValidExpiryDate && isValidCVV) {
                resolve({
                    success: true,
